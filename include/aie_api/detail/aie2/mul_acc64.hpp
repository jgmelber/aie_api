...
struct mul_bits_impl<MulOp, 64, 32, int32, 32, cint16>
{
    ...
    // Prefer underlying complex mul as cint32 * cint16 elementwise is natively supported
    // whereas int32 * int16 elementwise is emulated as the native mode is a 4x2x4 mmul
    using base_impl = mul_bits_impl<MulOp, 64, 64, cint32, 32, cint16>;

    static constexpr unsigned native_num_mul = base_impl::native_num_mul;
    ...
    static accum_type<Elems> run(const vector_type1<Elems> &v1, bool v1_sign, const vector_type2<Elems> &v2, bool v2_sign, const Acc &... acc)
    {
        ...
        utils::unroll_times<num_muls>([&](unsigned idx) __aie_inline {
                ...
                vector<cint32, elems_per_mul> tmp = tmp1.template grow<elems_per_mul * 2>(0)
                                                        .insert(1, tmp2)
                                                        .template cast_to<cint32>();
