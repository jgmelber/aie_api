...
struct mul_bits_impl<MulOp, 32, 16, bfloat16, 16, bfloat16>
{
    ...
    static constexpr auto get_mul_op()
    {
        ...
        else if constexpr (MulOp == MulMacroOp::NegMul)  return [](auto &&... args) __aie_inline { return ::negmul_elem_16_2_conf(args...); };
        else if constexpr (MulOp == MulMacroOp::Add_Mul) return [](auto &&... args) __aie_inline { return ::mac_elem_16_2_conf(args...); };
        else if constexpr (MulOp == MulMacroOp::Sub_Mul) return [](auto &&... args) __aie_inline { return ::msc_elem_16_2_conf(args...); };
    }
                                 ...
                                 bool zero_acc, const Acc &... acc)
    {
        ...
        utils::unroll_times<num_mul>([&](auto idx) __aie_inline {
            ...
            const vector_type<32> v2_ = v2.template grow_extract<16>(idx)
                                          .template grow<32>()
                                          .insert(1, z);
            const accum_type<16> tmp = mul_op(v1_, v2_,
    ...
    static accum_type<Elems> run(T a,                         bool a_sign,
                                 ...
                                 bool zero_acc, const Acc &... acc)
    {
        return run(broadcast<T, Elems>::run(a), a_sign, v, v_sign, sub_mul, sub_acc, zero_acc, acc...);
    }
    ...
    static accum_type<Elems> run(const vector_type<Elems> &v, bool v_sign,
                                 ...
                                 bool zero_acc, const Acc &... acc)
    {
        return run(v, v_sign, broadcast<T, Elems>::run(a), a_sign, sub_mul, sub_acc, zero_acc, acc...);
    }
};
...
struct mul_bits_impl<MulOp, 32, 32, float, 32, float>
{
    ...
    static constexpr auto get_mul_op()
    {
        if      constexpr (MulOp == MulMacroOp::Mul)     return [](auto &&... args) __aie_inline { return ::mul_elem_16_conf(args...); };
        else if constexpr (MulOp == MulMacroOp::NegMul)  return [](auto &&... args) __aie_inline { return ::mul_elem_16_conf(args...); };
        else if constexpr (MulOp == MulMacroOp::Add_Mul) return [](auto &&... args) __aie_inline { return ::mac_elem_16_conf(args...); };
        else if constexpr (MulOp == MulMacroOp::Sub_Mul) return [](auto &&... args) __aie_inline { return ::msc_elem_16_conf(args...); };
    }

    template <unsigned Elems, typename... Acc> requires((is_accum_v<Acc> && ...))
    __aie_inline
    static accum_type<Elems> run(const vector_type<Elems> &v1, const bool v1_sign,
                                 const vector_type<Elems> &v2, const bool v2_sign,
                                 bool sub_mul, bool sub_acc,
                                 bool zero_acc, const Acc &... acc)
    {
        constexpr auto mul_op = get_mul_op<Elems>();
    ...
    template <unsigned Elems, typename... Acc> requires((is_accum_v<Acc> && ...))
    __aie_inline
    static accum_type<Elems> run(T a,                         const bool a_sign,
                                 const vector_type<Elems> &v, const bool v_sign,
                                 bool sub_mul, bool sub_acc,
                                 bool zero_acc, const Acc &... acc)
    {
        return run(broadcast<T, Elems>::run(a), a_sign, v, v_sign, sub_mul, sub_acc, zero_acc, acc...);
    ...
    template <unsigned Elems, typename... Acc> requires((is_accum_v<Acc> && ...))
    __aie_inline
    static accum_type<Elems> run(const vector_type<Elems> &v, const bool v_sign,
                                 T a,                         const bool a_sign,
                                 bool sub_mul, bool sub_acc,
                                 bool zero_acc, const Acc &... acc)
    {
        return run(v, v_sign, broadcast<T, Elems>::run(a), a_sign, sub_mul, sub_acc, zero_acc, acc...);
...
struct mul_bits_cbf16_common
{
    ...
    static constexpr auto get_mul_op()
    {
        if      constexpr (MulOp == MulMacroOp::Mul)     return [](auto &&... args) __aie_inline { return ::mul_elem_8_2_conf(args...); };
        else if constexpr (MulOp == MulMacroOp::NegMul)  return [](auto &&... args) __aie_inline { return ::negmul_elem_8_2_conf(args...); };
        else if constexpr (MulOp == MulMacroOp::Add_Mul) return [](auto &&... args) __aie_inline { return ::mac_elem_8_2_conf(args...); };
        else if constexpr (MulOp == MulMacroOp::Sub_Mul) return [](auto &&... args) __aie_inline { return ::msc_elem_8_2_conf(args...); };
    }
                                 ...
                                 bool zero_acc, const Acc &... acc)
    {
        ...
        utils::unroll_times<num_mul>([&](auto idx) __aie_inline {
            ...
            const vector_type2<16> v2_ = v2.template grow_extract<8>(idx)
                                           .template grow<16>()
                                           .insert(1, z2);
            const accum_type<8> tmp = mul_op(v1_, v2_,
                                 ...
                                 bool zero_acc, const Acc &... acc)
    {
        ...
        utils::unroll_times<num_mul>([&](auto idx) __aie_inline {
            const accum_type<8> tmp = mul_op(v1.template grow_extract<8>(idx),
                                             ...
                                             utils::get_nth<0>(acc.template grow_extract<8>(idx), acc)...);
                                             //FIXME: CRVO-10674 missing conf support
                                             //utils::get_nth<0>(acc.template grow_extract<8>(idx), acc)...,
                                             //utils::get_nth<0>(zero_acc, acc)...,
                                             //sub_mul,
                                             //utils::get_nth<0>(sub_acc, acc)...);
