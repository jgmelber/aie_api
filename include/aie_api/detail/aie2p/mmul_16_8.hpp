...
template <typename TypeA, typename TypeB>
struct mmul_16_8<8, 2, 8, TypeA, TypeB, 32> : public C_block<TypeA, TypeB, 32, 64, 1>
{
    using vector_A_type = vector<TypeA, 16>;
    ...
    __aie_inline void mac(const vector_A_type &a, bool a_sign, const vector_B_type &b, bool b_sign)
    {
        this->data = ::mac_8x2_2x8_conf(a.template grow<32>(), a_sign, b.unpack_sign(b_sign).template grow<32>(), b_sign, this->data, this->zero, 0, 0, 0);
        this->zero = false;
    }

    __aie_inline void mul(const vector_A_type &a, bool a_sign, const vector_B_type &b, bool b_sign)
    {
        this->data = ::mul_8x2_2x8(a.template grow<32>(), a_sign, b.unpack_sign(b_sign).template grow<32>(), b_sign);
        this->zero = false;
    }
};
...
struct mmul_16_8<8, 4, 8, TypeA, TypeB, 32> : public C_block<TypeA, TypeB, 32, 64, 1>
{
    ...
    __aie_inline void mac(const vector_A_type &a, bool a_sign, const vector_B_type &b, bool b_sign)
    {
        auto [a_left, a_right] = interleave_unzip<TypeA, 32>::run(a, a, 2);
        auto b_bottom = shuffle_down<TypeB, 32>::run(b, 16);

        this->data = ::mac_8x2_2x8_conf(a_left,  a_sign, b.unpack_sign(b_sign), b_sign, this->data, this->zero, 0, 0, 0);
        this->data = ::mac_8x2_2x8_conf(a_right, a_sign, b_bottom.unpack_sign(b_sign), b_sign, this->data, 0, 0, 0, 0);
        this->zero = false;
    }
    ...
    __aie_inline void mul(const vector_A_type &a, bool a_sign, const vector_B_type &b, bool b_sign)
    {
        ...
        this->data = ::mul_8x2_2x8     (a_left,  a_sign, b.unpack_sign(b_sign), b_sign);
        this->data = ::mac_8x2_2x8_conf(a_right, a_sign, b_bottom.unpack_sign(b_sign), b_sign, this->data, 0, 0, 0, 0);
        this->zero = false;
    }
};
...
template <typename TypeA, typename TypeB, unsigned AccumBits>
struct mmul_16_8<4, 4, 8, TypeA, TypeB, AccumBits> : public C_block<TypeA, TypeB, 64, 32, 1>
{
    using vector_A_type = vector<TypeA, 16>;
    using vector_B_type = vector<TypeB, 32>;
    ...
    __aie_inline void mac(const vector_A_type &a, bool a_sign, const vector_B_type &b, bool b_sign)
    {
        this->data = ::mac_4x4_4x8_conf(a.template grow<32>(), a_sign, b.unpack_sign(b_sign), b_sign, this->data, this->zero, 0, 0, 0);
        this->zero = false;
    }

    __aie_inline void mul(const vector_A_type &a, bool a_sign, const vector_B_type &b, bool b_sign)
    {
        this->data = ::mul_4x4_4x8(a.template grow<32>(), a_sign, b.unpack_sign(b_sign), b_sign);
        this->zero = false;
    }
};
...
struct mmul_16_8<4, 8, 8, TypeA, TypeB, AccumBits> : public C_block<TypeA, TypeB, 64, 32, 1>
{
    ...
    __aie_inline void mac(const vector_A_type &a, bool a_sign, const vector_B_type &b, bool b_sign)
    {
        auto [a_left, a_right] = interleave_unzip<TypeA, 32>::run(a, a, 4);

        this->data = ::mac_4x4_4x8_conf(a_left,  a_sign, b.template extract<32>(0).unpack_sign(b_sign), b_sign, this->data, this->zero, 0, 0, 0);
        this->data = ::mac_4x4_4x8_conf(a_right, a_sign, b.template extract<32>(1).unpack_sign(b_sign), b_sign, this->data, 0, 0, 0, 0);
        this->zero = false;
    }
    ...
    __aie_inline void mul(const vector_A_type &a, bool a_sign, const vector_B_type &b, bool b_sign)
    {
        ...
        this->data = ::mul_4x4_4x8     (a_left,  a_sign, b.template extract<32>(0).unpack_sign(b_sign), b_sign);
        this->data = ::mac_4x4_4x8_conf(a_right, a_sign, b.template extract<32>(1).unpack_sign(b_sign), b_sign, this->data, 0, 0, 0, 0);
        this->zero = false;
    }
};
