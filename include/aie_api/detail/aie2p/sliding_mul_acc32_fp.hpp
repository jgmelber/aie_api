...
static constexpr auto sliding_mul_accfloat_get_mul_op()
{
    if constexpr (Lanes <= 32) {
        if      constexpr (MulOp == MulMacroOp::Mul)     return [](auto &&... args) __aie_inline { return ::mul_elem_32_conf(args..., /*sub_mul=*/0); };
        else if constexpr (MulOp == MulMacroOp::NegMul)  return [](auto &&... args) __aie_inline { return ::negmul_elem_32_conf(args..., /*sub_mul=*/0); };
        else if constexpr (MulOp == MulMacroOp::Add_Mul) return [](auto &&... args) __aie_inline { return ::mac_elem_32_conf(args..., /*sub_mul=*/0, /*sub_acc1=*/0); };
        else if constexpr (MulOp == MulMacroOp::Sub_Mul) return [](auto &&... args) __aie_inline { return ::msc_elem_32_conf(args..., /*sub_mul=*/0, /*sub_acc1=*/0); };
    }
    else if constexpr (Lanes == 64 && std::is_same_v<CoeffType, bfloat16>) {
        if      constexpr (MulOp == MulMacroOp::Mul)     return [](auto &&... args) __aie_inline { return ::mul_elem_64_conf(args..., /*sub_mul=*/0); };
        else if constexpr (MulOp == MulMacroOp::NegMul)  return [](auto &&... args) __aie_inline { return ::negmul_elem_64_conf(args..., /*sub_mul=*/0); };
        else if constexpr (MulOp == MulMacroOp::Add_Mul) return [](auto &&... args) __aie_inline { return ::mac_elem_64_conf(args..., /*sub_mul=*/0, /*sub_acc1=*/0); };
        else if constexpr (MulOp == MulMacroOp::Sub_Mul) return [](auto &&... args) __aie_inline { return ::msc_elem_64_conf(args..., /*sub_mul=*/0, /*sub_acc1=*/0); };
    }
}
...
template <unsigned Lanes, unsigned Points>
struct sliding_mul_bits_impl<Lanes, Points, 1, 1, 1, 32, 16, 16, float16, float16> : sliding_mul_bits_fp_common<Lanes, Points, float16> {};
#endif
template <unsigned Lanes, unsigned Points>
