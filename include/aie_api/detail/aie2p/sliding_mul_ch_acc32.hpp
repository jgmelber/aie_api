...
{
    if constexpr (MulOp == MulMacroOp::Mul)     return [](auto &&... args) __aie_inline { return ::mul_conv_8x8_8ch_conf(args..., /*sub_mul=*/0); };
    if constexpr (MulOp == MulMacroOp::NegMul)  return [](auto &&... args) __aie_inline { return ::negmul_conv_8x8_8ch_conf(args..., /*sub_mul=*/0); };
    if constexpr (MulOp == MulMacroOp::Add_Mul) return [](auto &&... args) __aie_inline { return ::mac_conv_8x8_8ch_conf(args..., /*shift16=*/0, /*sub_mul=*/0, /*sub_acc1=*/0); };
    if constexpr (MulOp == MulMacroOp::Sub_Mul) return [](auto &&... args) __aie_inline { return ::msc_conv_8x8_8ch_conf(args..., /*shift16=*/0, /*sub_mul=*/0, /*sub_acc1=*/0); };
}
                          ...
                          const Acc &... acc)
    {
        ...
        if constexpr (Points % columns_per_mul) {
            ...
            if constexpr (coeff_elems > 64)
                coeff2 = shuffle_down_rotate<coeff_type, coeff_elems>::run(coeff, coeff_start * Channels).template extract<64>(0);
            else
            ...
            auto data2 = data_lo.template grow<N_Data * 2>().insert(1, data_hi);

            auto [coeff_lo, coeff_hi] = interleave_zip<CoeffType, N_Coeff>::run(coeff, coeff, Channels);
            auto coeff2 = coeff_lo.template grow<N_Coeff * 2>().insert(1, coeff_hi);
            ...
            if constexpr (sizeof...(Acc) > 0) {
                return add_accum<accum_tag, Lanes>::run(acc..., zero_acc, accum_type(tmp_lo));
            }
            else {
        ...
        utils::unroll_for<unsigned, 0, std::max(1u, Lanes / 64)>([&](auto idx_y) __aie_inline {
            ...
            tmp = mul_op(shuffle_down_rotate<data_type, data_elems>::run(data.template grow_replicate<data_elems>(),
                         ...
                         data_sign,
                         shuffle_down_rotate<coeff_type, coeff_elems>::run(coeff.template grow_replicate<coeff_elems>(), coeff_start * Channels).template extract<64>(0),
                         coeff_sign,
            ...
            utils::unroll_for<unsigned, 1, num_mul>([&](auto idx) __aie_inline {
                tmp = mac_op(shuffle_down_rotate<data_type, data_elems>::run(data.template grow_replicate<data_elems>(),
                             ...
                             data_sign,
                             shuffle_down_rotate<coeff_type, coeff_elems>::run(coeff.template grow_replicate<coeff_elems>(), (coeff_start + columns_per_mul * idx) * Channels).template extract<64>(0),
                             coeff_sign,
                             tmp, false);
            });
